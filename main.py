...
async def brushTime(interaction: discord.Interaction):
    ...
    if update == False:
        ...
        await interaction.response.send_message(f'{horse_data[2]} enjoys being brushed. {PRONOUNS[horse_data[3],2]} coat is now spotless!')
...
async def vetServices(interaction: discord.Interaction, vet_services: int):
    ...
    if update == False:
        ...
        match vet_services:
            ...
            case 2:
                await interaction.response.send_message(f'The dental floating went well. {PRONOUNS[horse_data[3],2]} teeth are nice and flat now, no more mouth issues for {horse_data[2]}!')
            case 3:
